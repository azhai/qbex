...
void *emalloc(size_t n) {
  ...
  p = calloc(1, n);
  if (!p) die("emalloc, out of memory");
  return p;
}
...
void freeall(void) {
  ...
  for (;;) {
    for (pp = &pool[1]; pp < &pool[nptr]; pp++) free(*pp);
    pp = pool[0];
    if (!pp) break;
    free(pool);
    pool = pp;
...
void addins(Ins **pvins, uint *pnins, Ins *i) {
  if (i->op == Onop) return;
  vgrow(pvins, ++(*pnins));
  (*pvins)[(*pnins) - 1] = *i;
}

void addbins(Blk *b, Ins **pvins, uint *pnins) {
  Ins *i;

  for (i = b->ins; i < &b->ins[b->nins]; i++) addins(pvins, pnins, i);
}
...
uint32_t intern(char *s) {
  ...
  for (i = 0; i < n; i++)
    if (strcmp(s, b->str[i]) == 0) return h + (i << IBits);

  if (n == 1 << (32 - IBits)) die("interning table overflow");
  if (n == 0)
...
void igroup(Blk *b, Ins *i, Ins **i0, Ins **i1) {
  Ins *ib, *ie;
  ...
  switch (i->op) {
    ...
    case_Oarg:
      ...
      *i1 = i + 1;
      return;
    default:
      if (ispar(i->op)) goto case_Opar;
      if (isarg(i->op)) goto case_Oarg;
      *i0 = i;
      *i1 = i + 1;
      return;
  }
...
int cmpwlneg(int op) {
  if (INRANGE(op, Ocmpw, Ocmpw1)) return cmpneg(op - Ocmpw) + Ocmpw;
  if (INRANGE(op, Ocmpl, Ocmpl1)) return cmpneg(op - Ocmpl) + Ocmpl;
...
uint phiargn(Phi *p, Blk *b) {
  uint n;

  if (p)
    for (n = 0; n < p->narg; n++)
      if (p->blk[n] == b) return n;
  return -1;
}

Ref phiarg(Phi *p, Blk *b) {
  uint n;
...
Ref newtmp(char *prfx, int k, Fn *fn) {
  ...
  if (prfx) strf(fn->tmp[t].name, "%s.%d", prfx, ++n);
  fn->tmp[t].cls = k;
  fn->tmp[t].slot = -1;
  fn->tmp[t].nuse = +1;
  fn->tmp[t].ndef = +1;
...
void chuse(Ref r, int du, Fn *fn) {
  if (rtype(r) == RTmp) fn->tmp[r.val].nuse += du;
}
...
Ref newcon(Con *c0, Fn *fn) {
  ...
  for (i = 1; i < fn->ncon; i++) {
    c1 = &fn->con[i];
    if (c0->type == c1->type && symeq(c0->sym, c1->sym) &&
        c0->bits.i == c1->bits.i)
      return CON(i);
  }
...
Ref getcon(int64_t val, Fn *fn) {
  ...
  for (c = 1; c < fn->ncon; c++)
    if (fn->con[c].type == CBits && fn->con[c].bits.i == val) return CON(c);
  vgrow(&fn->con, ++fn->ncon);
...
int addcon(Con *c0, Con *c1, int m) {
  if (m != 1 && c1->type == CAddr) return 0;
  if (c0->type == CUndef) {
    *c0 = *c1;
    c0->bits.i *= m;
  } else {
    if (c1->type == CAddr) {
      if (c0->type == CAddr) return 0;
      c0->type = CAddr;
      c0->sym = c1->sym;
    }
...
void salloc(Ref rt, Ref rs, Fn *fn) {
  ...
  if (rtype(rs) == RCon) {
    ...
    if (sz < 0 || sz >= INT_MAX - 15) err("invalid alloc size %" PRId64, sz);
    sz = (sz + 15) & -16;
...
uint bscount(BSet *bs) {
  ...
  n = 0;
  for (i = 0; i < bs->nt; i++) n += popcnt(bs->t[i]);
  return n;
}
...
#define BSOP(f, op)                                 \
  void f(BSet *a, BSet *b) {                        \
    uint i;                                         \
                                                    \
    assert(a->nt == b->nt);                         \
    for (i = 0; i < a->nt; i++) a->t[i] op b->t[i]; \
  }
...
void dumpts(BSet *bs, Tmp *tmp, FILE *f) {
  ...
  fprintf(f, "[");
  for (t = Tmp0; bsiter(bs, &t); t++) fprintf(f, " %s", tmp[t].name);
  fprintf(f, " ]\n");
}
