  ...
  switch (rtype(r)) {
    ...
    case RTmp:
      *a = fn->tmp[r.val].alias;
      if (astack(a->type)) a->type = a->slot->type;
      assert(a->type != ABot);
      break;
    case RCon:
      c = &fn->con[r.val];
...
static void esc(Ref r, Fn *fn) {
  ...
  if (rtype(r) == RTmp) {
    a = &fn->tmp[r.val].alias;
    if (astack(a->type)) a->slot->type = AEsc;
  }
}
...
void fillalias(Fn *fn) {
  ...
  for (t = 0; t < fn->ntmp; t++) fn->tmp[t].alias.type = ABot;
  for (n = 0; n < fn->nblk; ++n) {
    b = fn->rpo[n];
    ...
    for (i = b->ins; i < &b->ins[b->nins]; ++i) {
      ...
      if (req(i->to, R) || a->type == AUnk)
        if (i->op != Oblit0) {
          if (!isload(i->op)) esc(i->arg[0], fn);
          if (!isstore(i->op))
            if (i->op != Oargc) esc(i->arg[1], fn);
        }
      if (i->op == Oblit0) {
        ++i;
        assert(i->op == Oblit1);
      ...
      if (isstore(i->op)) store(i->arg[1], storesz(i), fn);
    }
    if (b->jmp.type != Jretc) esc(b->jmp.arg, fn);
  }
  for (b = fn->start; b; b = b->link)
    for (p = b->phi; p; p = p->link)
      for (n = 0; n < p->narg; n++) esc(p->arg[n], fn);
}
