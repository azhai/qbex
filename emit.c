...
void emitlnk(char *n, Lnk *l, int s, FILE *f) {
  ...
  if (l->sec) {
    ...
    if (l->secf) fprintf(f, ",%s", l->secf);
  } else
    fputs(sec[l->thread != 0][s], f);
  fputc('\n', f);
  if (l->align) fprintf(f, ".balign %d\n", l->align);
  if (l->export) fprintf(f, ".globl %s%s\n", pfx, n);
  fprintf(f, "%s%s%s:\n", pfx, n, sfx);
}
  ...
  switch (d->type) {
    ...
    case DEnd:
      if (d->lnk->common) {
        ...
        if (d->lnk->align) fprintf(f, ",%d", d->lnk->align);
        fputc('\n', f);
      } else if (zero != -1) {
    ...
    default:
      if (zero != -1) {
        ...
        if (zero > 0) fprintf(f, "\t.fill %" PRId64 ",1,0\n", zero);
        zero = -1;
      }
      if (d->isstr) {
        if (d->type != DB) err("strings only supported for 'b' currently");
        fprintf(f, "\t.ascii %s\n", d->u.str);
      } else if (d->isref) {
...
void emitdbgfile(char *fn, FILE *f) {
  ...
  if (!file) file = vnew(0, sizeof *file, PHeap);
  vgrow(&file, ++nfile);
  file[nfile - 1] = id;
  curfile = nfile;
  fprintf(f, ".file %u %s\n", curfile, fn);
