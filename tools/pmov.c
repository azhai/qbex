...
int main() {
  ...
  for (tm = 0; tm < 1ull << (2 * NIReg); tm++) {
     ...
     */
    for (t = 0; t < NIReg; t++) switch ((tm >> (2 * t)) & 3) {
        case 0:
    ...
    nr = ip - ins;
    rm = (1ull << (nr + 1)) - 1;
    for (i = 0; i < nr; i++) reg[i] = i + 1;

    for (;;) {
      /* set registers on copies
       */
      for (i = 0, i1 = ins; i1 < ip; i1++, i++) i1->arg[0] = TMP(reg[i]);
       ...
       */
      for (i = 1; i <= NIReg; i++) val[i] = i;
      iexec(val);
      for (i1 = ins; i1 < ip; i1++) {
        t = i1->to.val;
        r = rfind(&mbeg, t);
        if (r != -1) assert(val[r] == i1->arg[0].val);
      }
      for (i = 0; i < mend.n; i++) {
        t = mend.t[i];
        r = mend.r[i];
...
#define validr(r) rtype(r) == RTmp &&r.val > 0 && r.val <= NIReg

static void iexec(int val[]) {
  Ins *i;
  int t;

  for (i = insb; i < curi; i++) switch (i->op) {
      default:
...
static void fail() {
  ...
  printf("\nIn registers: ");
  for (i = 0; i < mbeg.n; i++)
    printf("%s(r%d) ", tmp[mbeg.t[i]].name, mbeg.r[i]);
  printf("\n");
  printf("Parallel move:\n");
  for (i1 = ins; i1 < ip; i1++)
    printf("\t %s <- r%d\n", tmp[i1->to.val].name, i1->arg[0].val);
  replay();
  abort();
}

static void assert_test(char *s, int x) {
  if (x) return;
  if (re) abort();
...
static void bsinit_(BSet *bs, uint n) {
  n = (n + NBit - 1) / NBit;
  bs->nt = n;
