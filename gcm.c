...
static uint earlyins(Fn *fn, Blk *b, Ins *i) {
  uint b0, b1;
...
static void earlyblk(Fn *fn, uint bid) {
  Blk *b;
  Phi *p;
  Ins *i;
  uint n;
  ...
  for (p = b->phi; p; p = p->link)
    for (n = 0; n < p->narg; n++) schedearly(fn, p->arg[n]);
...
static uint lcabid(Fn *fn, uint bid1, uint bid2) {
  Blk *b;
...
static uint bestbid(Fn *fn, uint earlybid, uint latebid) {
  ...
  while (curb != earlyb) {
    curb = curb->idom;
    if (curb->loop < bestb->loop) bestb = curb;
  }
  return bestb->id;
}
...
static uint schedlate(Fn *fn, Ref r) {
  Tmp *t;
  Blk *b;
  Use *u;
  uint earlybid;
  ...
  if (rtype(r) != RTmp) return NOBID;

  t = &fn->tmp[r.val];
  if (t->visit) return t->gcmbid;
  ...
  if (t->def && !pinned(t->def)) t->gcmbid = bestbid(fn, earlybid, latebid);
...
static uint lateins(Fn *fn, Blk *b, Ins *i, Ref r) {
  uint latebid;
...
static uint latephi(Fn *fn, Phi *p, Ref r) {
  uint n;
  uint latebid;

  if (!p->narg) return NOBID; /* marked as unused */

  latebid = NOBID;
  for (n = 0; n < p->narg; n++)
    if (req(p->arg[n], r)) latebid = lcabid(fn, latebid, p->blk[n]->id);

  assert(latebid != NOBID);
  return latebid;
}

static uint latejmp(Blk *b, Ref r) {
  if (req(b->jmp.arg, R))
    return NOBID;
  else {
...
static void lateblk(Fn *fn, uint bid) {
  Blk *b;
  Phi **pp;
  Ins *i;
  ...
  for (i = b->ins; i < &b->ins[b->nins]; i++)
    if (pinned(i)) schedlate(fn, i->to);
}

static void addgcmins(Fn *fn, Ins *vins, uint nins) {
  Ins *i;
  Tmp *t;
  Blk *b;
...
static void gcmmove(Fn *fn) {
  ...
  for (t = fn->tmp; t < &fn->tmp[fn->ntmp]; t++) {
    if (t->def == 0) continue;
    if (t->bid == t->gcmbid) continue;
    i = t->def;
    if (pinned(i) && !canelim(i)) continue;
    assert(rtype(i->to) == RTmp);
    assert(t == &fn->tmp[i->to.val]);
    if (t->gcmbid != NOBID) addins(&vins, &nins, i);
    *i = (Ins){.op = Onop};
  }
...
/* dfs ordering */
static Ins *schedins(Fn *fn, Blk *b, Ins *i, Ins **pvins, uint *pnins) {
  Ins *i0, *i1;
  Tmp *t;
  uint n;
  ...
  for (i = i0; i < i1; i++)
    for (n = 0; n < 2; n++) {
      if (rtype(i->arg[n]) != RTmp) continue;
      t = &fn->tmp[i->arg[n].val];
      if (t->bid != b->id || !t->def) continue;
...
static int cheap(Ins *i) {
  int x;

  if (KBASE(i->cls) != 0) return 0;
  switch (i->op) {
    case Oneg:
    case Oadd:
    case Osub:
...
static void sinkref(Fn *fn, Blk *b, Ref *pr) {
  Ins i;
  Tmp *t;
  Ref r;

  if (rtype(*pr) != RTmp) return;
  t = &fn->tmp[pr->val];
  if (!t->def || t->bid == b->id || pinned(t->def) || !cheap(t->def)) return;
...
void gcm(Fn *fn) {
  ...
  for (bid = 0; bid < fn->nblk; bid++) earlyblk(fn, bid);
  for (bid = 0; bid < fn->nblk; bid++) lateblk(fn, bid);
