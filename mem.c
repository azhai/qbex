...
void promote(Fn *fn) {
  ...
  for (i = b->ins; i < &b->ins[b->nins]; i++) {
    if (Oalloc > i->op || i->op > Oalloc1) continue;
    /* specific to NAlign == 3 */
    assert(rtype(i->to) == RTmp);
    t = &fn->tmp[i->to.val];
    if (t->ndef != 1) goto Skip;
    k = -1;
    s = -1;
    for (u = t->use; u < &t->use[t->nuse]; u++) {
      if (u->type != UIns) goto Skip;
      l = u->u.ins;
      if (isload(l->op))
        if (s == -1 || s == loadsz(l)) {
          s = loadsz(l);
...
static inline int rovlap(Range r0, Range r1) {
...
static int slot(Slot **ps, int64_t *off, Ref r, Fn *fn, Slot *sl) {
  ...
  getalias(&a, r, fn);
  if (a.type != ALoc) return 0;
  t = &fn->tmp[a.base];
  if (t->visit < 0) return 0;
  *off = a.offset;
  *ps = &sl[t->visit];
  return 1;
}
...
static int scmp(const void *pa, const void *pb) {
  ...
  a = (Slot *)pa, b = (Slot *)pb;
  if (a->sz != b->sz) return b->sz - a->sz;
  return a->r.a - b->r.a;
}

static void maxrpo(Blk *hd, Blk *b) {
  if (hd->loop < (int)b->id) hd->loop = b->id;
}
...
void coalesce(Fn *fn) {
  ...
  for (n = fn->nblk - 1; n >= 0; n--) {
    ...
    if (b->jmp.type == Jretc) load(b->jmp.arg, -1, --ip, fn, sl);
  ...
  for (s = sl; s < &sl[nsl]; s++)
    for (n = 0; n < s->nst; n++)
      if (!rin(s->r, s->st[n].ip)) {
        i = s->st[n].i;
        if (i->op == Oblit0) *(i + 1) = (Ins){.op = Onop};
        *i = (Ins){.op = Onop};
      }
  ...
  if (debug['M']) {
    ...
    if (n) {
      fputs("\tkill [", stderr);
      for (m = 0; m < n; m++) fprintf(stderr, " %%%s", fn->tmp[stk[m]].name);
      fputs(" ]\n", stderr);
    }
  }
  while (n--) {
  ...
  for (n = 0; n < nsl; n++) {
    ...
    for (s = s0 + 1; s < &sl[nsl]; s++) {
      ...
      if (rovlap(r, s->r))
        ...
        for (m = n; &sl[m] < s; m++)
          if (sl[m].s == s0)
            if (rovlap(sl[m].r, s->r)) goto Skip;
      radd(&r, s->r.a);
      radd(&r, s->r.b - 1);
      s->s = s0;
      fused += s->sz;
  ...
  for (s = sl; s < &sl[nsl]; s++) {
    ...
    *t->def = (Ins){.op = Onop};
    ...
    for (u = t->use; u < &t->use[t->nuse]; u++) {
      ...
      arg = u->u.ins->arg;
      for (n = 0; n < 2; n++)
        if (req(arg[n], TMP(s->t))) arg[n] = TMP(s->s->t);
    }
  }
  ...
  for (s = sl; s < &sl[nsl]; s++) vfree(s->st);
  vfree(sl);
}
