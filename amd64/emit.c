...
static void emitcon(Con *con, E *e) {
  char *p, *l;
  ...
  switch (con->type) {
    case CAddr:
      ...
      if (con->sym.type == SThr) {
        ...
        fprintf(e->f, "%s%s", p, l);
      if (con->bits.i) fprintf(e->f, "%+" PRId64, con->bits.i);
      break;
    case CBits:
...
static void emitcopy(Ref r1, Ref r2, int k, E *e) {
  Ins icp;
    ...
    Ref:
      ...
      switch (rtype(ref)) {
        case RTmp:
          ...
          break;
        case RSlot:
          fprintf(e->f, "%d(%%%s)", slot(ref, e), regtoa(e->fp, SLong));
          break;
        case RMem:
        Mem:
          m = &e->fn->mem[ref.val];
          ...
          if (m->offset.type != CUndef) emitcon(&m->offset, e);
          fputc('(', e->f);
          if (!req(m->base, R))
            ...
            fprintf(e->f, "%%rip");
          if (!req(m->index, R))
            fprintf(e->f, ", %%%s, %d", regtoa(m->index.val, SLong), m->scale);
          fputc(')', e->f);
          break;
        case RCon:
          fputc('$', e->f);
    ...
    case 'M':
      ...
      switch (rtype(ref)) {
        ...
        case RCon:
          ...
          emitcon(&off, e);
          if (off.type == CAddr)
            if (off.sym.type != SThr || T.apple) fprintf(e->f, "(%%rip)");
          break;
        case RTmp:
          assert(isreg(ref));
  ...
  switch (i.op) {
    ...
    Table:
      ...
      for (o = 0;; o++) {
        ...
        if (omap[o].op == i.op)
          if (omap[o].cls == i.cls ||
              (omap[o].cls == Ki && KBASE(i.cls) == 0) || (omap[o].cls == Ka))
            break;
      }
    ...
    case Osub:
      ...
      goto Table;
    case Oneg:
      if (!req(i.to, i.arg[0])) emitf("mov%k %0, %=", &i, e);
      if (KBASE(i.cls) == 0)
        emitf("neg%k %=", &i, e);
      else
        fprintf(e->f, "\txorp%c %sfp%d(%%rip), %%%s\n", "xxsd"[i.cls], T.asloc,
                stashbits(negmask[i.cls], 16), regtoa(i.to.val, SLong));
      break;
    case Odiv:
    ...
    case Ocopy:
      ...
      if (req(i.to, R) || req(i.arg[0], R)) break;
      if (req(i.to, i.arg[0])) break;
      t0 = rtype(i.arg[0]);
      if (i.cls == Kl && t0 == RCon && e->fn->con[i.arg[0].val].type == CBits) {
      ...
      if (isreg(i.to) && t0 == RCon && e->fn->con[i.arg[0].val].type == CAddr) {
        emitf("lea%k %M0, %=", &i, e);
        break;
      }
    ...
    case Oaddr:
      if (!T.apple && rtype(i.arg[0]) == RCon &&
        ...
        fprintf(e->f, "\tleaq %s%s@tpoff", sym[0] == '"' ? "" : T.assym, sym);
        if (con->bits.i) fprintf(e->f, "%+" PRId64, con->bits.i);
    ...
    case Osalloc:
      ...
      if (!req(i.to, R)) emitcopy(i.to, TMP(RSP), Kl, e);
      break;
    case Oswap:
      if (KBASE(i.cls) == 0) goto Table;
...
static void framesz(E *e) {
  uint64_t i, o, f;

  /* specific to NAlign == 3 */
  o = 0;
  if (!e->fn->leaf) {
    for (i = 0, o = 0; i < NCLR; i++) o ^= e->fn->reg >> amd64_sysv_rclob[i];
    o &= 1;
  }
  f = e->fn->slot;
  f = (f + 3) & -4;
  if (f > 0 && e->fp == RSP && e->fn->salign == 4) f += 2;
...
#undef X
  ...
  if (!fn->leaf || fn->vararg || fn->dynalloc) {
    ...
    e->fp = RSP;
  framesz(e);
  if (e->fsz) fprintf(f, "\tsubq $%" PRIu64 ", %%rsp\n", e->fsz);
  if (fn->vararg) {
    o = -176;
  ...
  for (lbl = 0, b = fn->start; b; b = b->link) {
    if (lbl || b->npred > 1) {
      for (p = 0; p < b->npred; p++)
        if (b->pred[p]->id >= b->id) break;
      if (p != b->npred) fprintf(f, ".p2align 4\n");
      fprintf(f, "%sbb%d:\n", T.asloc, id0 + b->id);
    }
    for (i = b->ins; i != &b->ins[b->nins]; i++) emitins(*i, e);
    lbl = 1;
    switch (b->jmp.type) {
      case Jhlt:
        fprintf(f, "\tud2\n");
  ...
  id0 += fn->nblk;
  if (!T.apple) elf_emitfnfin(fn->name, f);
}
