...
static int hascon(Ref r, Con **pc, Fn *fn) {
  switch (rtype(r)) {
    case RCon:
      *pc = &fn->con[r.val];
      return 1;
...
static void fixarg(Ref *r, int k, Ins *i, Fn *fn) {
  ...
  } else if (T.apple && hascon(r0, &c, fn) && c->type == CAddr &&
             c->sym.type == SThr) {
    r1 = newtmp("isel", Kl, fn);
    if (c->bits.i) {
...
static void seladdr(Ref *r, Num *tn, Fn *fn) {
  Addr a;
  Ref r0;
...
static void sel(Ins i, Num *tn, Fn *fn) {
  Ref r0, r1, tmp[7];
  ...
  switch (i.op) {
    ...
    case Ourem:
      ...
      fixarg(&curi->arg[0], k, curi, fn);
      if (rtype(i.arg[1]) == RCon) emit(Ocopy, k, r0, i.arg[1], R);
      break;
    case Osar:
    case Oshr:
    case Oshl:
      r0 = i.arg[1];
      if (rtype(r0) == RCon) goto Emit;
      if (fn->tmp[r0.val].slot != -1)
        err("unlikely argument %%%s in %s", fn->tmp[r0.val].name,
            optab[i.op].name);
      i.arg[1] = TMP(RCX);
    ...
    case Oultof:
      ...
      if (k == Ks)
        ...
        kc = Kl, sh = 52;
      for (j = 0; j < 4; j++) tmp[j] = newtmp("utof", Kl, fn);
      for (; j < 7; j++) tmp[j] = newtmp("utof", kc, fn);
    ...
    Oftoui:
      ...
      for (j = 0; j < 4; j++) tmp[j] = newtmp("ftou", Kl, fn);
    ...
    case Oalloc16:
      ...
      break;
    default:
      if (isext(i.op)) goto case_OExt;
      if (isload(i.op)) goto case_Oload;
      if (iscmp(i.op, &kc, &x)) {
        switch (x) {
          case NCmpI + Cfeq:
...
static Ins *flagi(Ins *i0, Ins *i) {
  while (i > i0) {
    i--;
    if (amd64_op[i->op].zflag) return i;
    if (amd64_op[i->op].lflag) continue;
    return 0;
  }
  return 0;
}
...
static void seljmp(Blk *b, Fn *fn) {
  ...
  if (b->jmp.type == Jret0 || b->jmp.type == Jjmp || b->jmp.type == Jhlt)
    return;
  ...
  if (!fi || !req(fi->to, r)) {
     ...
     */
    if (t->nuse == 1) emit(Ocopy, Kw, R, r, R);
    b->jmp.type = Jjf + Cine;
  }
}
...
static int opn(int op, int l, int r) {
  static uchar Oaddtbl[91] = {
      2, 2, 2, 4, 4,  5,  6,  6,  8,  8, 4, 4, 9,  10, 9, 7,  7, 5,  8,
      9, 5, 4, 4, 12, 10, 12, 12, 12, 4, 4, 9, 10, 9,  9, 12, 9, 11, 11,
      5, 8, 9, 5, 12, 9,  5,  7,  7,  5, 8, 9, 5,  12, 9, 5,  5, 11, 11,
      5, 8, 9, 5, 12, 9,  5,  5,  5,  4, 4, 9, 10, 9,  9, 12, 9, 9,  9,
      9, 9, 7, 7, 5,  8,  9,  5,  12, 9, 5, 5, 5,  9,  5,
  };
  int t;
...
static int refn(Ref r, Num *tn, Con *con) {
  int64_t n;

  switch (rtype(r)) {
    case RTmp:
      if (!tn[r.val].n) tn[r.val].n = 2;
      return tn[r.val].n;
    case RCon:
...
static uchar *matcher[] = {
    [Pbi1] = (uchar[]){1, 3, 1, 3, 2, 0},
    [Pbis] = (uchar[]){5, 1, 8, 5, 27, 1, 5, 1, 2, 5, 13, 3, 1, 1, 3,
                       3, 3, 2, 0, 1,  3, 3, 3, 2, 3, 1,  0, 1, 29},
    [Pob] = (uchar[]){1, 3, 0, 3, 1, 0},
    [Pobi1] =
        (uchar[]){5,  3,  9, 9, 10, 33, 12, 35, 45, 1,  5,  3, 11, 9, 7, 9,  4,
                  9,  17, 1, 3, 0,  3,  1,  3,  2,  0,  3,  1, 1,  3, 0, 34, 1,
                  37, 1,  5, 2, 5,  7,  2,  7,  8,  37, 29, 1, 3,  0, 1, 32},
    [Pobis] =
        (uchar[]){5, 2, 10, 7, 11, 19, 49, 1,  1, 3,  3, 3, 2,  1, 3,  0,  3,
                  1, 0, 1,  3, 0,  5,  1,  8,  5, 25, 1, 5, 1,  2, 5,  13, 3,
                  1, 1, 3,  3, 3,  2,  0,  1,  3, 3,  3, 2, 26, 1, 51, 1,  5,
                  1, 6, 5,  9, 1,  3,  0,  51, 3, 1,  1, 3, 0,  45},
    [Pois] = (uchar[]){1, 3, 0, 1, 3, 3, 3, 2, 0},
};

/* end of generated code */

static void anumber(Num *tn, Blk *b, Con *con) {
  Ins *i;
  Num *n;
...
static Ref adisp(Con *c, Num *tn, Ref r, Fn *fn, int s) {
  Ref v[2];
  int n;
...
static int amatch(Addr *a, Num *tn, Ref r, Fn *fn) {
  ...
  if (*p < 0 && co.type != CUndef)
    if (amatch(a, tn, rb, fn)) return addcon(&a->offset, &co, 1);
  if (!req(ro, R)) {
    assert(rtype(ro) == RCon);
    c = &fn->con[ro.val];
    if (!addcon(&co, c, 1)) return 0;
  }
  if (!req(rs, R)) {
  ...
  *a = (Addr){co, rb, ri, s};
  ...
  if (rtype(ri) == RTmp)
    if (fn->tmp[ri.val].slot != -1) {
      if (a->scale != 1 || fn->tmp[rb.val].slot != -1) return 0;
      a->base = ri;
      a->index = rb;
    }
  if (!req(a->base, R)) {
...
void amd64_isel(Fn *fn) {
  ...
  for (al = Oalloc, n = 4; al <= Oalloc1; al++, n *= 2)
    for (i = b->ins; i < &b->ins[b->nins]; i++)
      if (i->op == al) {
        if (rtype(i->arg[0]) != RCon) break;
        sz = fn->con[i->arg[0].val].bits.i;
        if (sz < 0 || sz >= INT_MAX - 15)
          err("invalid alloc size %" PRId64, sz);
        sz = (sz + n - 1) & -n;
        sz /= 4;
        if (sz > INT_MAX - fn->slot) die("alloc too large");
  ...
  for (b = fn->start; b; b = b->link) {
    ...
    for (sb = (Blk *[3]){b->s1, b->s2, 0}; *sb; sb++)
      for (p = (*sb)->phi; p; p = p->link) {
        for (a = 0; p->blk[a] != b; a++) assert(a + 1 < p->narg);
        fixarg(&p->arg[a], p->cls, 0, fn);
      }
    memset(num, 0, n * sizeof num[0]);
    anumber(num, b, fn->con);
    seljmp(b, fn);
    for (i = &b->ins[b->nins]; i != b->ins;) sel(*--i, num, fn);
