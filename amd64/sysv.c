...
static int argsclass(Ins *i0, Ins *i1, AClass *ac, int op, AClass *aret,
  ...
  varc = 0;
  envc = 0;
  for (i = i0, a = ac; i < i1; i++, a++) switch (i->op - op + Oarg) {
      case Oarg:
        if (KBASE(i->cls) == 0)
          pn = &nint;
        else
  ...
  if (varc && envc) err("sysv abi does not support variadic env calls");
...
bits amd64_sysv_argregs(Ref r, int p[2]) {
  ...
  nf = (r.val >> 8) & 15;
  ra = (r.val >> 12) & 1;
  for (j = 0; j < ni; j++) b |= BIT(amd64_sysv_rsave[j]);
  for (j = 0; j < nf; j++) b |= BIT(XMM0 + j);
  if (p) {
    p[0] = ni + ra;
    p[1] = nf;
  }
...
static void selcall(Fn *fn, Ins *i0, Ins *i1, RAlloc **rap) {
  ...
  for (stk = 0, a = &ac[i1 - i0]; a > ac;)
    if ((--a)->inmem) {
      if (a->align > 4) err("sysv abi requires alignments of 16 or less");
      stk += a->size;
      if (a->align == 4) stk += stk & 15;
    }
  stk += stk & 15;
  if (stk) {
  ...
  if (!req(i1->arg[1], R)) {
    if (aret.inmem) {
      ...
      ca += retr(reg, &aret);
      if (aret.size > 8) emit(Ocopy, aret.cls[1], aret.ref[1], reg[1], R);
  ...
  for (i = i0, a = ac; i < i1; i++, a++) {
    if (i->op >= Oarge || a->inmem) continue;
...
static int selpar(Fn *fn, Ins *i0, Ins *i1) {
  ...
  for (i = i0, a = ac; i < i1; i++, a++) {
    if (i->op != Oparc || a->inmem) continue;
    if (a->size > 8) {
      r = newtmp("abi", Kl, fn);
  ...
  if (!req(R, env)) emit(Ocopy, Kl, env, TMP(RAX), R);
...
void amd64_sysv_abi(Fn *fn) {
  ...
  do {
    ...
    if (b == fn->start)
      for (; ral; ral = ral->link) emiti(ral->i);
