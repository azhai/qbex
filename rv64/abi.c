...
bits rv64_retregs(Ref r, int p[2]) {
  ...
  b = 0;
  while (ngp--) b |= BIT(A0 + ngp);
  while (nfp--) b |= BIT(FA0 + nfp);
  return b;
}
...
static int fpstruct(Typ *t, int off, Class *c) {
  ...
  if (t->isunion) return -1;

  for (f = *t->fields; f->type != FEnd; f++)
    if (f->type == FPad)
      off += f->len;
    else if (f->type == FTyp) {
      if (fpstruct(&typ[f->len], off, c) == -1) return -1;
    } else {
      n = c->nfp + c->ngp;
      if (n == 2) return -1;
      switch (f->type) {
...
static void typclass(Class *c, Typ *t, int fpabi, int *gp, int *fp) {
  ...
  if (t->align > 4) err("alignments larger than 16 are not supported");
  ...
  for (i = i0, c = carg; i < i1; i++, c++) {
    switch (i->op) {
        ...
        *c->cls = i->cls;
        if (!vararg && KBASE(i->cls) == 1 && nfp > 0) {
          ...
          *c->reg = *fp++;
          ...
          if (KBASE(i->cls) == 1) c->class |= Cfpint;
          ngp--;
          *c->reg = *gp++;
        } else
          c->class |= Cstk1;
      ...
      case Oargc:
        ...
        if (c->nfp > 0)
          if (c->nfp >= nfp || c->ngp >= ngp) typclass(c, t, 0, gp, fp);
        assert(c->nfp <= nfp);
        if (c->ngp <= ngp) {
          ngp -= c->ngp;
          nfp -= c->nfp;
...
static void selcall(Fn *fn, Ins *i0, Ins *i1, Insl **ilp) {
  ...
  if (!req(i1->arg[1], R)) typclass(&cr, &typ[i1->arg[1].val], 1, gpreg, fpreg);

  cty = argsclass(i0, i1, ca, cr.class & Cptr);
  stk = 0;
  for (i = i0, c = ca; i < i1; i++, c++) {
    if (i->op == Oargv) continue;
    if (c->class & Cptr) {
      i->arg[0] = newtmp("abi", Kl, fn);
      stkblob(i->arg[0], c->type, fn, ilp);
      i->op = Oarg;
    }
    if (c->class & Cstk1) stk += 8;
    if (c->class & Cstk2) stk += 8;
  }
  stk += stk & 15;
  if (stk) emit(Osalloc, Kl, R, getcon(-stk, fn), R);
  ...
  if (cr.class & Cptr) /* struct return argument */
    emit(Ocopy, Kl, TMP(A0), i1->to, R);

  /* move arguments into registers */
  for (i = i0, c = ca; i < i1; i++, c++) {
    if (i->op == Oargv || c->class & Cstk1) continue;
    if (i->op == Oargc) {
      ldregs(c, i->arg[1], fn);
  ...
  r = newtmp("abi", Kl, fn);
  for (i = i0, c = ca; i < i1; i++, c++) {
    if (i->op == Oargv || !(c->class & Cstk)) continue;
    if (i->op == Oarg) {
      r1 = newtmp("abi", Kl, fn);
