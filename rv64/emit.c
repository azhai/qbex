...
static int64_t slot(Ref r, Fn *fn) {
  int s;
  ...
  for (;;) {
    ...
    switch ((c = *s++)) {
      ...
      case '?':
        ...
        break;
      case 'k':
        if (i->cls != Kl) fputc(clschr[i->cls], f);
        break;
      case '=':
      case '0':
        r = c == '=' ? i->to : i->arg[0];
...
static void loadaddr(Con *c, char *rn, FILE *f) {
  char off[32];
...
static void fixmem(Ref *pr, Fn *fn, FILE *f) {
  Ref r;
  int64_t s;
  Con *c;
...
static void emitins(Ins *i, Fn *fn, FILE *f) {
  ...
  switch (i->op) {
    ...
    case Ocall:
      switch (rtype(i->arg[0])) {
        case RCon:
          ...
          if (con->type != CAddr || con->sym.type != SGlo || con->bits.i)
            goto Invalid;
          fprintf(f, "\tcall %s\n", str(con->sym.id));
          break;
        case RTmp:
    ...
    case Osalloc:
      ...
      if (!req(i->to, R)) emitf("mv %=, sp", i, fn, f);
      break;
    case Odbgloc:
    ...
    switch (b->jmp.type) {
      ...
      case Jjnz:
        ...
        fprintf(f, "\tb%sz %s, .L%d\n", neg ? "ne" : "eq",
                rname[b->jmp.arg.val], id0 + b->s2->id);
        goto Jmp;
    }
  }
  id0 += fn->nblk;
