...
#define F(cf, hi, id, co, as, im, ic, lg, cv, pn)                       \
  .canfold = cf, .hasid = hi, .idval = id, .commutes = co, .assoc = as, \
  .idemp = im, .cmpeqwl = ic, .cmplgtewl = lg, .eqval = cv, .pinned = pn
...
static void lexinit(void) {
  ...
  if (done) return;
  for (i = 0; i < NPubOp; ++i)
    if (optab[i].name) kwmap[i] = optab[i].name;
...
static int lex(void) {
  ...
  switch (c) {
    ...
    case '+':
      return Tplus;
    case 's':
      if (fscanf(inf, "_%f", &tokval.flts) != 1) break;
      return Tflts;
    case 'd':
      if (fscanf(inf, "_%lf", &tokval.fltd) != 1) break;
      return Tfltd;
    case '%':
      t = Ttmp;
      c = fgetc(inf);
  ...
  Quoted:
    ...
    for (i = 1;; i++) {
      c = fgetc(inf);
      if (c == EOF) err("unterminated string");
...
Alpha:
  ...
  i = 0;
  do {
    if (i >= NString - 1) err("identifier too long");
    tok[i++] = c;
    c = fgetc(inf);
...
static Ref tmpref(char *v) {
  ...
  for (; tmph[i]; i = (i + 1) & (tmphcap - 1)) {
    t = tmph[i];
    if (strcmp(curf->tmp[t].name, v) == 0) return TMP(t);
  }
  t = curf->ntmp;
  tmph[i] = t;
  newtmp(0, Kx, curf);
...
static int findtyp(int i) {
  while (--i >= 0)
    if (strcmp(tokval.str, typ[i].name) == 0) return i;
...
static int parserefl(int arg) {
  ...
  expect(Tlparen);
  while (peek() != Trparen) {
    if (curi - insb >= NIns) err("too many instructions");
    if (!arg && vararg) err("no parameters allowed after '...'");
    switch (peek()) {
      case Tdots:
        if (vararg) err("only one '...' allowed");
        vararg = 1;
        if (arg) {
          *curi = (Ins){.op = Oargv};
          curi++;
        ...
        goto Next;
      case Tenv:
        if (hasenv) err("only one environment allowed");
        hasenv = 1;
        env = 1;
        next();
        k = Kl;
    ...
    r = parseref();
    if (req(r, R)) err("invalid argument");
    if (!arg && rtype(r) != RTmp) err("invalid function parameter");
    if (env)
      if (arg)
...
static PState parseline(PState ps) {
  ...
  t = nextnl();
  if (ps == PLbl && t != Tlbl && t != Trbrace) err("label or } expected");
  switch (t) {
    case Ttmp:
      r = tmpref(tokval.str);
      expect(Teq);
    ...
    case Tret:
      ...
      if (peek() == Tnl)
        ...
        r = parseref();
        if (req(r, R)) err("invalid return value");
        curb->jmp.arg = r;
      }
      goto Close;
    case Tjmp:
    ...
    case Tjnz:
      curb->jmp.type = Jjnz;
      r = parseref();
      if (req(r, R)) err("invalid argument for jnz jump");
      curb->jmp.arg = r;
      expect(Tcomma);
    Jump:
      expect(Tlbl);
    ...
    case Odbgloc:
      ...
      if (arg[0].val != tokval.num) err("line number too big");
      if (peek() == Tcomma) {
        next();
        expect(Tint);
        arg[1] = INT(tokval.num);
        if (arg[1].val != tokval.num) err("column number too big");
      } else
        arg[1] = INT(0);
      goto Ins;
  }
  ...
  if (op == Tloadw) op = Oloadsw;
  if (op >= Tloadl && op <= Tloadd) op = Oload;
  if (op == Talloc1 || op == Talloc2) op = Oalloc;
  if (op == Ovastart && !curf->vararg)
    err("cannot use vastart in non-variadic function");
  if (k >= Ksb) err("size class must be w, l, s, or d");
  i = 0;
  if (peek() != Tnl)
    for (;;) {
      if (i == NPred) err("too many arguments");
      if (op == Tphi) {
        expect(Tlbl);
        blk[i] = findblk(tokval.str);
      }
      arg[i] = parseref();
      if (req(arg[i], R)) err("invalid instruction argument");
      i++;
      t = peek();
      if (t == Tnl) break;
      if (t != Tcomma) err(", or end of line expected");
      next();
    }
  next();
  switch (op) {
    case Tphi:
      if (ps != PPhi || curb == curf->start) err("unexpected phi instruction");
      ...
      *plink = phi;
      ...
      return PPhi;
    case Tblit:
      if (curi - insb >= NIns - 1) err("too many instructions");
      memset(curi, 0, 2 * sizeof(Ins));
      curi->op = Oblit0;
      curi->arg[0] = arg[0];
      curi->arg[1] = arg[1];
      curi++;
      if (rtype(arg[2]) != RCon) err("blit size must be constant");
      ...
      return PIns;
    default:
      if (op >= NPubOp) err("invalid instruction");
    Ins:
      if (curi - insb >= NIns) err("too many instructions");
      curi->op = op;
      curi->cls = k;
...
static void typecheck(Fn *fn) {
  ...
  for (b = fn->start; b; b = b->link) {
    for (p = b->phi; p; p = p->link) fn->tmp[p->to.val].cls = p->cls;
  ...
  for (b = fn->start; b; b = b->link) {
    bszero(pb);
    for (n = 0; n < b->npred; n++) bsset(pb, b->pred[n]->id);
    ...
    for (i = b->ins; i < &b->ins[b->nins]; i++)
      for (n = 0; n < 2; n++) {
        ...
        r = i->arg[n];
        t = &fn->tmp[r.val];
        if (k == Ke) err("invalid instruction type in %s", optab[i->op].name);
...
static Fn *parsefn(Lnk *lnk) {
  ...
  if (peek() != Tglo)
    ...
    rcls = K0;
  if (next() != Tglo) err("function name expected");
  strncpy(curf->name, tokval.str, NString - 1);
  curf->vararg = parserefl(0);
  if (nextnl() != Tlbrace) err("function body must start with {");
  ps = PLbl;
  do ps = parseline(ps);
  while (ps != PEnd);
  if (!curb) err("empty function");
  if (curb->jmp.type == Jxxx) err("last block misses jump");
...
static void parsefields(Field *fld, Typ *ty, int t) {
  ...
  if (t != Trbrace) err(", or } expected");
  fld[n].type = FEnd;
  a = 1 << al;
  if (sz < ty->size) sz = ty->size;
  ty->size = (sz + a - 1) & -a;
  ty->align = al;
}

static void parsetyp(void) {
  Typ *ty;
  int t, al;
  uint n;
  ...
  ty->align = -1;
  ty->size = 0;
  if (nextnl() != Ttyp || nextnl() != Teq) err("type name and then = expected");
  strcpy(ty->name, tokval.str);
  t = nextnl();
  if (t == Talign) {
    if (nextnl() != Tint) err("alignment expected");
    for (al = 0; tokval.num /= 2; al++);
    ty->align = al;
    t = nextnl();
  }
  if (t != Tlbrace) err("type body must start with {");
  t = nextnl();
  if (t == Tint) {
    ty->isdark = 1;
    ty->size = tokval.num;
    if (ty->align == -1) err("dark types need alignment");
    if (nextnl() != Trbrace) err("} expected");
    return;
  }
  n = 0;
  ...
  if (t == Tlbrace) {
    ty->isunion = 1;
    do {
      if (t != Tlbrace) err("invalid union member");
      vgrow(&ty->fields, n + 1);
...
static void parsedatref(Dat *d) {
  ...
  if (t == Tplus) {
    next();
    if (next() != Tint) err("invalid token after offset in ref");
...
static void parsedat(void cb(Dat *), Lnk *lnk) {
  ...
  if (nextnl() != Tglo || nextnl() != Teq) err("data name, then = expected");
  ...
  if (t != Tlbrace) err("expected data contents in { .. }");
  for (;;) {
    switch (nextnl()) {
      default:
...
static int parselnk(Lnk *lnk) {
  ...
  for (haslnk = 0;; haslnk = 1) switch ((t = nextnl())) {
      case Texport:
        lnk->export = 1;
        break;
      case Tthread:
      ...
      case Tcommon:
        ...
        break;
      case Tsection:
        if (lnk->sec) err("only one section allowed");
        if (next() != Tstr) err("section \"name\" expected");
        lnk->sec = tokval.str;
        if (peek() == Tstr) {
          next();
          lnk->secf = tokval.str;
        ...
        break;
      default:
        if (t == Tfunc && lnk->thread) err("only data may have thread linkage");
...
void parse(FILE *f, char *path, void dbgfile(char *), void data(Dat *),
           void func(Fn *)) {
  Lnk lnk;
  uint n;
  ...
  for (;;) {
    ...
    switch (parselnk(&lnk)) {
      ...
      case Teof:
        for (n = 0; n < ntyp; n++)
          if (typ[n].nunion) vfree(typ[n].fields);
        vfree(typ);
        return;
    }
  }
...
static void printcon(Con *c, FILE *f) {
  switch (c->type) {
    case CUndef:
      break;
    case CAddr:
      if (c->sym.type == SThr) fprintf(f, "thread ");
      fprintf(f, "$%s", str(c->sym.id));
      if (c->bits.i) fprintf(f, "%+" PRIi64, c->bits.i);
      break;
    case CBits:
      if (c->flt == 1)
  ...
  for (b = fn->start; b; b = b->link) {
    ...
    switch (b->jmp.type) {
      ...
      case Jretc:
        ...
        if (b->jmp.type == Jretc) fprintf(f, ", :%s", typ[fn->retty].name);
        fprintf(f, "\n");
        break;
      case Jhlt:
        fprintf(f, "\thlt\n");
        break;
      case Jjmp:
        if (b->s1 != b->link) fprintf(f, "\tjmp @%s\n", b->s1->name);
        break;
      default:
        fprintf(f, "\t%s ", jtoa[b->jmp.type]);
        if (b->jmp.type == Jjnz) {
