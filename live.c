...
void liveon(BSet *v, Blk *b, Blk *s) {
  ...
  for (p = s->phi; p; p = p->link)
    if (rtype(p->to) == RTmp) bsclr(v, p->to.val);
  for (p = s->phi; p; p = p->link)
    for (a = 0; a < p->narg; a++)
      if (p->blk[a] == b)
    ...
    for (i = &b->ins[b->nins]; i != b->ins;) {
      if ((--i)->op == Ocall && rtype(i->arg[1]) == RCall) {
        ...
        for (k = 0; k < 2; k++) {
           ...
           * they are live: */
          nlv[k] += T.nrsave[k];
          if (nlv[k] > b->nlive[k]) b->nlive[k] = nlv[k];
        }
      ...
      if (!req(i->to, R)) {
        ...
        t = i->to.val;
        if (bshas(b->in, t)) nlv[KBASE(f->tmp[t].cls)]--;
        bsset(b->gen, t);
        bsclr(b->in, t);
      }
      for (k = 0; k < 2; k++) switch (rtype(i->arg[k])) {
          case RMem:
