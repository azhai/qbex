...
static int usewidthle(Fn *fn, Ref r, int w) {
  Ext e;
  Tmp *t;
  Use *u;
  Phi *p;
  ...
  for (u = t->use; u < &t->use[t->nuse]; u++) {
    switch (u->type) {
      case UPhi:
         ...
         * Phi.to is set to R */
        if (p->visit || req(p->to, R)) continue;
        p->visit = 1;
      ...
      case UIns:
        ...
        assert(i != 0);
        if (i->op == Ocopy)
          if (usewidthle(fn, i->to, w)) continue;
        if (ext(i, &e)) {
          if (e.usew <= w) continue;
          if (usewidthle(fn, i->to, w)) continue;
        }
        if (i->op == Oand) {
          if (req(r, i->arg[0]))
          ...
          if (isconbits(fn, rc, &v) && bitwidth(v) <= w) continue;
          break;
        }
        break;
      default:
...
static int defwidthle(Fn *fn, Ref r, int w) {
  Ext e;
  Tmp *t;
  Phi *p;
  Ins *i;
  ...
  if (isconbits(fn, r, &v) && bitwidth(v) <= w) return 1;
  if (rtype(r) != RTmp) return 0;
  t = &fn->tmp[r.val];
  if (t->cls != Kw) return 0;
  ...
  i = t->def;
  if (i->op == Ocopy) return defwidthle(fn, i->arg[0], w);
  if (i->op == Oshr || i->op == Osar) {
    if (isconbits(fn, i->arg[1], &v))
      if (0 < v && v <= 32) {
        if (i->op == Oshr && w + v >= 32) return 1;
        if (w < 32) {
          if (i->op == Osar)
            w = min(31, w + v);
          else
  ...
  if (iscmp(i->op, &x, &x)) return w >= 1;
  if (i->op == Oand) {
    if (defwidthle(fn, i->arg[0], w) || defwidthle(fn, i->arg[1], w)) return 1;
    return 0;
  }
...
static int isw1(Fn *fn, Ref r) { return defwidthle(fn, r, 1); }
  ...
  for (i = b->ins; i < &b->ins[b->nins]; i++) {
    ...
    if (i->cls == Kw)
      if (usewidthle(fn, i->to, 16)) {
        ext.op = Oextuh;
        if (usewidthle(fn, i->to, 8)) ext.op = Oextub;
...
Ref copyref(Fn *fn, Blk *b, Ins *i) {
  ...
  if (i->op == Ocopy) return i->arg[0];

  /* op identity value */
  if (optab[i->op].hasid && KBASE(i->cls) == 0 /* integer only - fp NaN! */
      && req(i->arg[1], con01[optab[i->op].idval]) &&
      (!optab[i->op].cmpeqwl || isw1(fn, i->arg[0])))
    return i->arg[0];

  /* idempotent op with identical args */
  if (optab[i->op].idemp && req(i->arg[0], i->arg[1])) return i->arg[0];
  ...
  if (optab[i->op].cmpeqwl && req(i->arg[1], CON_Z) &&
      zeroval(fn, b, i->arg[0], argcls(i, 0), &z))
    return con01[optab[i->op].eqval ^ z ^ 1];

  /* redundant and mask */
  if (i->op == Oand && isconbits(fn, i->arg[1], &v) &&
      (v > 0 && ((v + 1) & v) == 0) && defwidthle(fn, i->arg[0], bitwidth(v)))
    return i->arg[0];

  if (i->cls == Kw && (i->op == Oextsw || i->op == Oextuw)) return i->arg[0];
  ...
  if (ext(i, &e) && rtype(i->arg[0]) == RTmp) {
     ...
     * a narrower temp */
    if (KWIDE(i->cls) > KWIDE(t->cls)) return R;

    w = Wsb + (i->op - Oextsb);
    if (BIT(w) & extcpy[t->width]) return i->arg[0];

    /* avoid eliding extensions of params
     * inserted in the start block; their
     * point is to make further extensions
     * redundant */
    if ((!t->def || !ispar(t->def->op)) && usewidthle(fn, i->to, e.usew))
      return i->arg[0];

    if (defwidthle(fn, i->arg[0], e.nopw)) return i->arg[0];
  }

  return R;
}

static int phieq(Phi *pa, Phi *pb) {
  Ref r;
  uint n;
  ...
  for (n = 0; n < pa->narg; n++) {
    ...
    if (!req(pa->arg[n], r)) return 0;
  }
  return 1;
}

Ref phicopyref(Fn *fn, Blk *b, Phi *p) {
  Blk *d, **s;
  Phi *p1;
  uint n, c;
  ...
  for (n = 0; n < p->narg - 1; n++)
    if (!req(p->arg[n], p->arg[n + 1])) break;
  if (n == p->narg - 1) return p->arg[n];
   ...
   * dominating jnz arg */
  d = b->idom;
  if (p->narg != 2 || d->jmp.type != Jjnz || !isw1(fn, d->jmp.arg)) return R;

  s = (Blk *[]){0, 0};
  for (n = 0; n < 2; n++)
    for (c = 0; c < 2; c++)
      if (req(p->arg[n], con01[c])) s[c] = p->blk[n];

  /* if s1 ends with a jnz on either b
   * or s2; the inference below is wrong
   * without the jump type checks */
  if (d->s1 == s[1] && d->s2 == s[0] && d->s1->jmp.type == Jjmp &&
      d->s2->jmp.type == Jjmp)
    return d->jmp.arg;
