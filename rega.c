...
static Ref ralloctry(RMap *m, int t, int try) {
  ...
  if (r == -1 || bshas(m->b, r)) {
    ...
    for (r = r0; r < r1; r++)
      if (!(regs & BIT(r))) goto Found;
    for (r = r0; r < r1; r++)
      if (!bshas(m->b, r)) goto Found;
    die("no more regs");
  }
Found:
  radd(m, t, r);
...
static int rfree(RMap *m, int t) {
  ...
  if (!bshas(m->b, t)) return -1;
  for (i = 0; m->t[i] != t; i++) assert(i + 1 < m->n);
  r = m->r[i];
  bsclr(m->b, t);
  bsclr(m->b, r);
  m->n--;
...
static void pmadd(Ref src, Ref dst, int k) {
  if (npm == Tmp0) die("cannot have more moves than registers");
  pm[npm].src = src;
  pm[npm].dst = dst;
  pm[npm].cls = k;
  npm++;
...
static int pmrec(enum PMStat *status, int i, int *k) {
  ...
  *k |= pm[i].cls;
  for (j = 0; j < npm; j++)
    if (req(pm[j].dst, pm[i].src)) break;
...
static void pmgen(void) {
  ...
  for (i = 0; i < npm; i++)
    if (status[i] == ToMove) pmrec(status, i, (int[]){pm[i].cls});
}
...
static void move(int r, Ref to, RMap *m) {
  ...
  if (bshas(m->b, r)) {
    ...
    assert(r1 != r);
    for (n = 0; m->r[n] != r; n++) assert(n + 1 < m->n);
    t = m->t[n];
    rfree(m, t);
    bsset(m->b, r);
    ralloc(m, t);
...
static Ins *dopm(Blk *b, Ins *i, RMap *m) {
  ...
  if (i != b->ins && (i - 1)->op == Ocall) {
    ...
    for (r = 0; T.rsave[r] >= 0; r++)
      if (!(BIT(T.rsave[r]) & def)) move(T.rsave[r], R, m);
  }
  ...
  for (ip = i; ip < i1; ip++) {
    if (!req(ip->to, R)) rfree(m, ip->to.val);
    r = ip->arg[0].val;
    if (rfind(m, r) == -1) radd(m, r, r);
  }
  pmgen();
  return i;
}
...
static void doblk(Blk *b, RMap *cur) {
  ...
  if (rtype(b->jmp.arg) == RTmp) b->jmp.arg = ralloc(cur, b->jmp.arg.val);
  curi = &insb[NIns];
  for (i1 = &b->ins[b->nins]; i1 != b->ins;) {
    ...
    switch (i->op) {
      case Ocall:
        ...
        for (r = 0; T.rsave[r] >= 0; r++)
          if (!(BIT(T.rsave[r]) & rs)) rfree(cur, T.rsave[r]);
        break;
      case Ocopy:
        if (regcpy(i)) {
          curi++;
    ...
    for (x = 0, nr = 0; x < 2; x++) switch (rtype(i->arg[x])) {
        case RMem:
          m = &mem[i->arg[x].val];
          if (rtype(m->base) == RTmp) insert(&m->base, ra, nr++);
          if (rtype(m->index) == RTmp) insert(&m->index, ra, nr++);
          break;
        case RTmp:
          insert(&i->arg[x], ra, nr++);
          break;
      }
    for (r = 0; r < nr; r++) *ra[r] = ralloc(cur, ra[r]->val);
    if (i->op == Ocopy && req(i->to, i->arg[0])) curi++;
    ...
    if (rf != -1 && (t = cur->w[rf]) != 0)
      if (!bshas(cur->b, rf) && *hint(t) == rf && (rt = rfree(cur, t)) != -1) {
        ...
        cur->w[rf] = 0;
        for (r = 0; r < nr; r++)
          if (req(*ra[r], TMP(rt))) *ra[r] = TMP(rf);
...
static int carve(const void *a, const void *b) {
  ...
  ba = *(Blk **)a;
  bb = *(Blk **)b;
  if (ba->loop == bb->loop) return ba->id > bb->id ? -1 : ba->id < bb->id;
...
static int prio2(int t1, int t2) {
  ...
  if ((*hint(t1) ^ *hint(t2)) < 0) return *hint(t1) != -1 ? +1 : -1;
...
void rega(Fn *fn) {
  ...
  for (bp = blk, b = fn->start; b; b = b->link) *bp++ = b;
