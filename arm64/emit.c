  ...
  if (r == SP) {
    ...
    switch (k) {
      default:
        die("invalid class");
      case Ks:
        sprintf(buf, "s%d", r - V0 + offset);
        break;
      case Kd:
        sprintf(buf, "d%d", r - V0 + offset);
        break;
      case Kx:
...
static uint64_t slot(Ref r, E *e) {
  int s;

  s = rsval(r);
  if (s == -1) return 16 + e->frame;
  if (s < 0) {
...
static void loadaddr(Con *c, char *rn, E *e) {
  char *p, *l, *s;
  ...
  for (; *s; s++) switch (*s) {
      ...
      case 'S':
        ...
        break;
      case 'O':
        if (c->bits.i) /* todo, handle large offsets */
          fprintf(e->f, "+%" PRIi64, c->bits.i);
        break;
    }
}

static void loadcon(Con *c, int r, int k, E *e) {
  char *rn;
  int64_t n;
  int w, sh;
...
static void emitins(Ins *i, E *e) {
  ...
  switch (i->op) {
    default:
      if (isload(i->op)) fixarg(&i->arg[0], loadsz(i), e);
      if (isstore(i->op)) fixarg(&i->arg[1], storesz(i), e);
    Table:
    ...
    case Oaddr:
      ...
      break;
    case Ocall:
      if (rtype(i->arg[0]) != RCon) goto Table;
      c = &e->fn->con[i->arg[0].val];
    ...
    case Osalloc:
      ...
      if (!req(i->to, R)) emitf("mov %=, sp", i, e);
      break;
    case Odbgloc:
...
static void framelayout(E *e) {
  ...
  for (o = 0, r = arm64_rclob; *r >= 0; r++) o += 1 & (e->fn->reg >> *r);
  f = e->fn->slot;
  f = (f + 3) & -4;
  o += o & 1;
...
#undef X
  ...
  e = &(E){.f = out, .fn = fn};
  if (T.apple) e->fn->lnk.align = 4;
  emitfnlnk(e->fn->name, &e->fn->lnk, e->f);
  fputs("\thint\t#34\n", e->f);
  framelayout(e);

  if (e->fn->vararg && !T.apple) {
    for (n = 7; n >= 0; n--) fprintf(e->f, "\tstr\tq%d, [sp, -16]!\n", n);
    for (n = 7; n >= 0; n -= 2)
  ...
  for (lbl = 0, b = e->fn->start; b; b = b->link) {
    if (lbl || b->npred > 1) fprintf(e->f, "%s%d:\n", T.asloc, id0 + b->id);
    ...
    switch (b->jmp.type) {
      ...
      case Jret0:
        ...
        if (e->fn->dynalloc) fputs("\tmov sp, x29\n", e->f);
        o = e->frame + 16;
        if (e->fn->vararg && !T.apple) o += 192;
        if (o <= 504)
      ...
      default:
        c = b->jmp.type - Jjf;
        if (c < 0 || c > NCmp) die("unhandled jump %d", b->jmp.type);
        if (b->link == b->s2) {
          t = b->s1;
          b->s1 = b->s2;
          b->s2 = t;
  ...
  id0 += e->fn->nblk;
  if (!T.apple) elf_emitfnfin(fn->name, out);
}
