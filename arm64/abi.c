...
static int isfloatv(Typ *t, char *cls) {
  ...
  for (n = 0; n < t->nunion; n++)
    for (f = t->fields[n]; f->type != FEnd; f++) switch (f->type) {
        case Fs:
          if (*cls == Kd) return 0;
          *cls = Ks;
          break;
        case Fd:
          ...
          *cls = Kd;
          break;
        case FTyp:
          if (isfloatv(&typ[f->len], cls)) break;
          /* fall through */
        default:
          return 0;
      }
...
static void typclass(Class *c, Typ *t, int *gp, int *fp) {
  ...
  if (t->align > 3) err("alignments larger than 8 are not supported");
...
static int argsclass(Ins *i0, Ins *i1, Class *carg) {
  ...
  ngp = 8;
  nfp = 8;
  for (i = i0, c = carg; i < i1; i++, c++) switch (i->op) {
      case Oargsb:
      case Oargub:
      case Oparsb:
      case Oparub:
      ...
      case Oarg:
        c->size = 8;
        if (T.apple && !KWIDE(i->cls)) c->size = 4;
      Scalar:
        c->align = c->size;
        *c->cls = i->cls;
        if (va) {
...
bits arm64_retregs(Ref r, int p[2]) {
  ...
  b = 0;
  while (ngp--) b |= BIT(R0 + ngp);
  while (nfp--) b |= BIT(V0 + nfp);
  return b;
}
...
static uint align(uint x, uint al) { return (x + al - 1) & -al; }
...
static void selcall(Fn *fn, Ins *i0, Ins *i1, Insl **ilp) {
  ...
  stk = align(stk, 16);
  rstk = getcon(stk, fn);
  if (stk) emit(Oadd, Kl, TMP(SP), TMP(SP), rstk);
  ...
  if (cty & (1 << 13)) /* struct return argument */
    emit(Ocopy, Kl, TMP(R8), i1->to, R);

  for (i = i0, c = ca; i < i1; i++, c++) {
    if ((c->class & Cstk) != 0) continue;
    if (i->op == Oarg || i->op == Oarge)
      emit(Ocopy, *c->cls, TMP(*c->reg), i->arg[0], R);
    if (i->op == Oargc) ldregs(c->reg, c->cls, c->nreg, i->arg[1], fn);
  }
  ...
  if (stk) emit(Osub, Kl, TMP(SP), TMP(SP), rstk);
...
static Params selpar(Fn *fn, Ins *i0, Ins *i1) {
  ...
  t = tmp;
  for (i = i0, c = ca; i < i1; i++, c++) {
    if (i->op != Oparc || (c->class & (Cptr | Cstk))) continue;
    sttmps(t, c->cls, c->nreg, i->to, fn);
    stkblob(i->to, c, fn, &il);
    t += c->nreg;
  }
  for (; il; il = il->link) emiti(il->i);
  ...
  return (Params){
      .stk = align(off, 8), .ngp = (cty >> 5) & 15, .nfp = (cty >> 9) & 15};
}
...
static void chpred(Blk *b, Blk *bp, Blk *bp1) {
  ...
  for (p = b->phi; p; p = p->link) {
    for (a = 0; p->blk[a] != bp; a++) assert(a + 1 < p->narg);
    p->blk[a] = bp1;
  }
}

static void apple_selvaarg(Fn *fn, Blk *b, Ins *i) {
  Ref ap, stk, stk8, c8;
...
static void arm64_selvaarg(Fn *fn, Blk *b, Ins *i) {
  ...
  */
  ...
  b0->s1 = b->s1;
  b0->s2 = b->s2;
  if (b->s1) chpred(b->s1, b, b0);
  if (b->s2 && b->s2 != b->s1) chpred(b->s2, b, b0);
...
static void apple_selvastart(Fn *fn, Params p, Ref ap) {
  Ref off, stk, arg;
...
static void arm64_selvastart(Fn *fn, Params p, Ref ap) {
  Ref r0, r1, rsave;
...
void arm64_abi(Fn *fn) {
  ...
  b = fn->start;
  do {
    if (!(b = b->link)) b = fn->start; /* do it last */
    if (b->visit) continue;
    curi = &insb[NIns];
    selret(b, fn);
    for (i = &b->ins[b->nins]; i != b->ins;) switch ((--i)->op) {
        default:
          emiti(*i);
          break;
        case Ocall:
          for (i0 = i; i0 > b->ins; i0--)
            if (!isarg((i0 - 1)->op)) break;
          selcall(fn, i0, i, &il);
          i = i0;
          break;
        case Ovastart:
