...
void filluse(Fn *fn) {
  ...
  for (t = Tmp0; t < fn->ntmp; t++) {
    ...
    tmp[t].phi = 0;
    tmp[t].width = WFull;
    if (tmp[t].use == 0) tmp[t].use = vnew(0, sizeof(Use), PFn);
  }
  for (b = fn->start; b; b = b->link) {
    ...
    for (i = b->ins; i < &b->ins[b->nins]; i++) {
      if (!req(i->to, R)) {
        ...
        w = WFull;
        if (isparbh(i->op)) w = Wsb + (i->op - Oparsb);
        if (isload(i->op) && i->op != Oload) w = Wsb + (i->op - Oloadsb);
        if (isext(i->op)) w = Wsb + (i->op - Oextsb);
        if (iscmp(i->op, &x, &x)) w = Wub;
        if (w == Wsw || w == Wuw)
          if (i->cls == Kw) w = WFull;
        t = i->to.val;
        tmp[t].width = w;
    ...
    if (rtype(b->jmp.arg) == RTmp) adduse(&tmp[b->jmp.arg.val], UJmp, b);
  }
}
...
static void phiins(Fn *fn) {
  ...
  for (t = Tmp0; t < nt; t++) {
    fn->tmp[t].visit = 0;
    if (fn->tmp[t].phi != 0) continue;
    if (fn->tmp[t].ndef == 1) {
      ok = 1;
      defb = fn->tmp[t].bid;
      use = fn->tmp[t].use;
      for (n = fn->tmp[t].nuse; n--; use++) ok &= use->bid == defb;
      if (ok || defb == fn->start->id) continue;
    }
    bszero(u);
    k = Kx;
    bp = be;
    for (b = fn->start; b; b = b->link) {
      ...
      for (i = b->ins; i < &b->ins[b->nins]; i++) {
        if (!req(r, R)) {
          if (req(i->arg[0], TMP(t))) i->arg[0] = r;
          if (req(i->arg[1], TMP(t))) i->arg[1] = r;
        }
        if (req(i->to, TMP(t))) {
          if (!bshas(b->out, t)) {
            r = refindex(t, fn);
            ...
            if (clsmerge(&k, i->cls)) die("invalid input");
          }
        }
      }
      if (!req(r, R) && req(b->jmp.arg, TMP(t))) b->jmp.arg = r;
    }
    bscopy(defs, u);
    while (bp != be) {
      fn->tmp[t].visit = t;
...
static void rendef(Ref *r, Blk *b, Name **stk, Fn *fn) {
  ...
  t = r->val;
  if (req(*r, R) || !fn->tmp[t].visit) return;
  r1 = refindex(t, fn);
...
static void renblk(Blk *b, Name **stk, Fn *fn) {
  ...
  for (p = b->phi; p; p = p->link) rendef(&p->to, b, stk, fn);
  for (i = b->ins; i < &b->ins[b->nins]; i++) {
    for (m = 0; m < 2; m++) {
      t = i->arg[m].val;
      if (rtype(i->arg[m]) == RTmp)
        if (fn->tmp[t].visit) i->arg[m] = getstk(t, b, stk);
    }
    rendef(&i->to, b, stk, fn);
  }
  t = b->jmp.arg.val;
  if (rtype(b->jmp.arg) == RTmp)
    if (fn->tmp[t].visit) b->jmp.arg = getstk(t, b, stk);
  succ[0] = b->s1;
  ...
  for (s = b->dom; s; s = s->dlink) renblk(s, stk, fn);
}
...
void ssa(Fn *fn) {
  ...
  if (debug['N']) {
    ...
    for (b1 = fn->start; b1; b1 = b1->link) {
      ...
      for (b = b1->dom; b; b = b->dlink) fprintf(stderr, " %s", b->name);
      fprintf(stderr, "\n");
    }
  }
  fillfron(fn);
...
void ssacheck(Fn *fn) {
  ...
  for (t = &fn->tmp[Tmp0]; t - fn->tmp < fn->ntmp; t++) {
    if (t->ndef > 1) err("ssa temporary %%%s defined more than once", t->name);
  ...
  for (b = fn->start; b; b = b->link) {
    for (p = b->phi; p; p = p->link) {
      ...
      for (u = t->use; u < &t->use[t->nuse]; u++) {
        bu = fn->rpo[u->bid];
        if (u->type == UPhi) {
          if (phicheck(u->u.phi, b, r)) goto Err;
