...
static void aggreg(Blk *hd, Blk *b) {
  ...
  for (k = 0; k < 2; k++)
    if (b->nlive[k] > hd->nlive[k]) hd->nlive[k] = b->nlive[k];
}
...
static void limit(BSet *b, int k, BSet *f) {
  ...
  for (i = 0; i < k && i < nt; i++) bsset(b, tarr[i]);
  for (; i < nt; i++) slot(tarr[i]);
}
...
static void sethint(BSet *u, bits r) {
  ...
  for (t = Tmp0; bsiter(u, &t); t++) tmp[phicls(t, tmp)].hint.m |= r;
}
...
static void reloads(BSet *u, BSet *v) {
  ...
  for (t = Tmp0; bsiter(u, &t); t++)
    if (!bshas(v, t)) emit(Oload, tmp[t].cls, TMP(t), slot(t), R);
}

static void store(Ref r, int s) {
  if (s != -1) emit(Ostorew + tmp[r.val].cls, 0, R, r, SLOT(s));
}
...
static Ins *dopm(Blk *b, Ins *i, BSet *v) {
  ...
  if (i != b->ins && (i - 1)->op == Ocall) {
    ...
    for (n = 0, r = 0; T.rsave[n] >= 0; n++) r |= BIT(T.rsave[n]);
...
void spill(Fn *fn) {
  ...
  for (t = 0; t < ntmp; t++) {
    k = 0;
    if (t >= T.fpr0 && t < T.fpr0 + T.nfpr) k = 1;
    if (t >= Tmp0) k = KBASE(tmp[t].cls);
    bsset(mask[k], t);
  }
  ...
  for (bp = &fn->rpo[fn->nblk]; bp != fn->rpo;) {
    ...
    s2 = b->s2;
    hd = 0;
    if (s1 && s1->id <= b->id) hd = s1;
    if (s2 && s2->id <= b->id)
      if (!hd || s2->id >= hd->id) hd = s2;
    if (hd) {
      /* back-edge */
      bszero(v);
      ...
      bscopy(v, b->out);
      if (rtype(b->jmp.arg) == RCall) v->t[0] |= T.retregs(b->jmp.arg, 0);
    }
    ...
    for (i = &b->ins[b->nins]; i != b->ins;) {
      ...
      j = T.memargs(i->op);
      for (n = 0; n < 2; n++)
        if (rtype(i->arg[n]) == RMem) j--;
      for (n = 0; n < 2; n++) switch (rtype(i->arg[n])) {
          case RMem:
            t = i->arg[n].val;
            m = &fn->mem[t];
