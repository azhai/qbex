...
static void cast(Ref *r, int cls, Loc *l) {
  ...
  if (KWIDE(cls0) < KWIDE(cls)) {
    if (cls0 == Ks) *r = iins(Kw, Ocast, *r, R, l);
    *r = iins(Kl, Oextuw, *r, R, l);
    if (cls == Kd) *r = iins(Kd, Ocast, *r, R, l);
  } else {
    if (cls0 == Kd && cls != Kl) *r = iins(Kl, Ocast, *r, R, l);
    if (cls0 != Kd || cls != Kw) *r = iins(cls, Ocast, *r, R, l);
  }
}
...
static Ref def(Slice sl, bits msk, Blk *b, Ins *i, Loc *il) {
  ...
  while (i > b->ins) {
    ...
    switch (alias(sl.ref, sl.off, sl.sz, r1, sz, &off, curf)) {
      case MustAlias:
        ...
        if ((msk1 & msk) == 0) continue;
        if (i->op == Oblit0) {
          r = def(sl1, MASK(sz), b, i, il);
          if (req(r, R)) goto Load;
        }
        if (off) {
          cls1 = cls;
          if (op == Oshr && off + sl.sz > 4) cls1 = Kl;
          cast(&r, cls1, il);
        ...
        if ((msk & ~msk1) != 0) {
          ...
          if (req(r1, R)) goto Load;
          r = iins(cls, Oor, r, r1, il);
        }
        if (msk == msks) cast(&r, sl.cls, il);
        return r;
      case MayAlias:
        if (ld)
          continue;
  ...
  if (b->npred == 0) goto Load;
  if (b->npred == 1) {
    bp = b->pred[0];
    assert(bp->loop >= il->blk->loop);
    l = *il;
    if (bp->s2) l.type = LNoLoad;
...
static int icmp(const void *pa, const void *pb) {
  ...
  a = (Insert *)pa;
  b = (Insert *)pb;
  if ((c = a->bid - b->bid)) return c;
  if (a->isphi && b->isphi) return 0;
  if (a->isphi) return -1;
  if (b->isphi) return +1;
  if ((c = a->off - b->off)) return c;
  return a->num - b->num;
}
