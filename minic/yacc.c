...
void first(TSet *ts, Sym *stnc, TSet *last) {
  ...
  if (is[f].nul) first(ts, stnc + 1, last);
  tsunion(ts, &is[f].fst);
}
...
void iclose(Item *i) {
  ...
  do {
    ...
    for (n = 0; n < i->nt; n++) {
      ...
      s = *rem++;
      if (s < Sym0 || s == S) continue;
      r = rfind(s);
      if (!r) die("some non-terminals are not defined");
      tszero(&t1.lk);
      ...
      if (m)
        ...
        smap[s - Sym0] = m;
        for (; r - rs < nrl && r->lhs == s; r++, m++) {
          if (m >= MaxTm) die("too many terms in item");
          t1.rule = r;
          i->ts[m] = t1;
        }
        i->nt = m;
...
void igoto(Item *i, Sym s) {
  ...
  for (n = 0, t = i->ts; n < i->nt; n++, t++) {
    if (t->rule->rhs[t->dot] != s) continue;
    t1 = &i0.ts[i0.nt++];
    *t1 = *t;
    t1->dot++;
  }
...
int icmp(Item *a, Item *b) {
  ...
  mb = tb + b->nt;
  for (;;) {
    if (ta == ma || ta->dot == 0) return -(tb < mb && tb->dot);
    if (tb == mb || tb->dot == 0) return +(ta < ma && ta->dot);
    if ((c = tcmp(ta++, tb++))) return c;
  }
}
...
int stadd(Item **pi) {
  ...
  if (hi < nst && icmp(st[hi], i) == 0) {
    chg = 0;
    i1 = st[hi];
    for (n = 0; n < i->nt; n++) chg |= tsunion(&i1->ts[n].lk, &i->ts[n].lk);
    i1->dirty |= chg;
    *pi = i1;
    return chg;
  } else {
    st = realloc(st, ++nst * sizeof st[0]);
    if (!st) die("out of memory");
...
int resolve(Rule *r, Sym s, int st) {
  if (!r->prec || !is[s].prec) {
  conflict:
    if (fgrm) fprintf(fgrm, srs, st, is[s].name);
    srconf++;
    return ARight;
  }
  if (r->prec == is[s].prec) {
    if (is[s].assoc == ANone) goto conflict;
    return is[s].assoc;
...
void tblset(int *tbl, Item *i, Term *t) {
  ...
  if (s != S) {
    ...
    /* reduce */
    for (s = 0; s < ntk; s++) {
      if (!GetBit(t->lk.t, s)) continue;
      /* default to shift if conflict occurs */
      if (!tbl[s])
        act = ALeft;
      else if (tbl[s] < 0) {
        if (fgrm) fprintf(fgrm, rrs, i->id - 1, is[s].name);
        rrconf++;
        act = ARight;
      } else
...
void setdef(Row *r, int w, int top) {
  ...
  r->def = def;
  if (max != 0) /* zero out the most frequent entry */
    for (n = 0; n < w; n++)
      if (r->t[n] == def) {
        r->t[n] = 0;
        r->ndef++;
...
void tblgen() {
  ...
  for (n = 0; n < nst; n++) {
    ...
    for (i = st[n], m = 0; m < i->nt; m++) tblset(r->t, i, &i->ts[m]);
    setdef(r, ntk, -1);
  ...
  for (n = MaxTk; n < nsy; n++) {
    ...
    for (m = 0; m < nst; m++)
      if (st[m]->gtbl[n]) r->t[m] = st[m]->gtbl[n]->id;
    setdef(r, nst, nst + 1);
  }
}
...
void actgen() {
  ...
  for (n = 0; n < nst * nsy; n++) chk[n] = -1;
  /* fill in actions */
  for (n = 0; n < nst; n++) o[n] = &as[n];
  qsort(o, nst, sizeof o[0], prcmp);
  for (n = 0; n < nst; n++) {
    r = o[n];
    dsp = 0;
    for (m = 0; m < ntk && r->t[m] == 0; m++) dsp--;
  retrya:
    ...
    for (t = 0; t < ntk; t++)
      if (r->t[t]) {
        chk[dsp + t] = t;
        act[dsp + t] = r->t[t];
        if (dsp + t >= actsz) actsz = dsp + t + 1;
      }
  }
  /* fill in gotos */
  nnt = nsy - MaxTk;
  gdsp = yalloc(nnt, sizeof gdsp[0]);
  for (n = 0; n < nnt; n++) o[n] = &gs[n];
  qsort(o, nnt, sizeof o[0], prcmp);
  for (n = 0; n < nnt; n++) {
    r = o[n];
    dsp = 0;
    for (m = 0; m < nst && r->t[m] == 0; m++) dsp--;
  retryg:
    for (t = m; t < nst; t++)
...
void tblout() {
  ...
  for (n = 0; n < nrl; n++) o[n] = slen(rs[n].rhs);
  aout("yyr1", o, nrl);
  for (n = 0; n < nrl; n++) o[n] = rs[n].lhs - MaxTk;
  aout("yyr2", o, nrl);
  for (n = 0; n < nst; n++) o[n] = as[n].def;
  aout("yyadef", o, nst);
  ...
  for (n = 0; n < 128; n++) {
    ...
    for (m = 0; m < ntk; m++)
      if (is[m].name[0] == '\'')
        if (is[m].name[1] == n) assert(!o[n]), o[n] = m;
  }
  m = 128;
  for (n = 1; n < ntk; n++) {
    if (is[n].name[0] == '\'') continue;
    fprintf(fout, "#define %s %d\n", is[n].name, m);
    if (fhdr) fprintf(fhdr, "#define %s %d\n", is[n].name, m);
    o[m++] = n;
  }
  aout("yytrns", o, m);
  if (fhdr) {
...
void stdump() {
  ...
  for (r = rs; r - rs < nrl; r++) {
    ...
    for (s1 = r->rhs; *s1 != S; s1++) fprintf(fgrm, " %s", is[*s1].name);
  }
  fprintf(fgrm, "\n");
  for (m = 0; m < nst; m++) {
    fprintf(fgrm, "\nState %d:\n", m);
    ...
    if (ar->def != -1) fprintf(fgrm, "  * reduce with rule %d\n", ar->def);
  }
}
...
int nexttk() {
  ...
  switch (c) {
    ...
    case '\'':
      ...
      idnt[2] = '\'';
      idnt[3] = 0;
      if (fgetc(fin) != '\'') die("syntax error, invalid char token");
      return TTokchr;
  }
  p = idnt;
  while (istok(c)) {
    *p++ = c;
    if (p - idnt >= IdntSz - 1) die("identifier too long");
    c = fgetc(fin);
  }
  *p = 0;
  if (strcmp(idnt, "%") == 0)
  ...
  for (n = 0; words[n].name; n++)
    if (strcmp(idnt, words[n].name) == 0) return words[n].tok;
  return TIdnt;
}
...
char *cpycode() {
  ...
  while (nest) {
    ...
    if (in) {
      ...
      if (c == '{') nest++;
      if (c == '}') nest--;
      if (c == EOF) die("syntax error, unclosed code block");
      if (c == '\n') lineno++;
    }
    if (pos >= len)
      if (!(s = realloc(s, len = 2 * len + 1))) die("out of memory");
    s[pos++] = c;
  }
  s[pos] = 0;
  return s;
...
int gettype(char *type) {
  ...
  tk = nexttk();
  if (tk == TLangle) {
    if (nexttk() != TIdnt) die("syntax error, ident expected after <");
    strcpy(type, idnt);
    if (nexttk() != TRangle) die("syntax error, unclosed <");
    return nexttk();
  } else {
    type[0] = 0;
    return tk;
...
Sym findsy(char *name, int add) {
  ...
  for (n = 0; n < nsy; n++) {
    if (n == ntk) {
      if (name[0] == '\'') {
        if (ntk >= MaxTk) die("too many tokens");
        ntk++;
    ...
    if (strcmp(is[n].name, name) == 0) return n;
  }
  if (add) {
    if (nsy >= MaxTk + MaxNt) die("too many non-terminals");
...
void getdecls() {
  ...
  for (;;) switch (tk) {
      case TStart:
        tk = nexttk();
        if (tk != TIdnt) die("syntax error, ident expected after %start");
        sstart = findsy(idnt, 1);
        if (sstart < ntk) die("%start cannot specify a token");
        tk = nexttk();
        break;
      case TUnion:
        tk = nexttk();
        if (tk != TLBrack) die("syntax error, { expected after %union");
      ...
      addtoks:
        ...
        while (tk == TIdnt || tk == TTokchr) {
          si = 0;
          n = findsy(idnt, 0);
          if (n >= MaxTk && n < nsy) die("non-terminal redeclared as token");
          if (n == nsy) {
            if (ntk >= MaxTk) die("too many tokens");
            n = ntk++;
          }
      ...
      case TType:
        tk = gettype(type);
        if (type[0] == 0) die("syntax error, type expected");
        while (tk == TIdnt) {
          si = 0;
          n = findsy(idnt, 1);
          if (n < ntk) die("token redeclared as non-terminal");
          if (n == nsy) {
            nsy++;
          }
          si = &is[n];
      ...
      case TLL:
        ...
        for (;;) {
          c = fgetc(fin);
          if (c == EOF) die("syntax error, unclosed %{");
          if (c == '%') {
            c1 = fgetc(fin);
            if (c1 == '}') {
              fputs("\n", fout);
...
void getgram() {
  ...
  for (;;) {
    tk = nexttk();
    if (tk == TPP || tk == TEof) {
      if (sstart == S) die("syntax error, empty grammar");
      r = &rs[nrl++];
      r->lhs = Sym0;
      r->rhs[0] = sstart;
      r->rhs[1] = 0;
    ...
    if (tk != TIdnt) die("syntax error, production rule expected");
    if (nexttk() != TColon)
      die("syntax error, colon expected after production's head");
    hd = findsy(idnt, 1);
    if (sstart == S) sstart = hd;
    do {
      if (nrl >= MaxRl - 1) die("too many rules");
      r = &rs[nrl++];
      r->lhs = hd;
      r->act = 0;
      p = r->rhs;
      while ((tk = nexttk()) == TIdnt || tk == TTokchr || tk == TPrec) {
        ...
        s = findsy(idnt, 1);
        *p++ = s;
        if (s < ntk && is[s].prec > 0) r->prec = is[s].prec;
        if (p - r->rhs >= MaxRhs - 1) die("production rule too long");
      }
      *p = S;
      if (tk == TLBrack) {
        r->actln = lineno;
    ...
    if (tk != TSemi) die("syntax error, ; or | expected");
  }
}
  ...
  while ((c = *p++)) switch (c) {
      ...
      case '$':
        ...
        if (c == '$') {
          ...
          ty = tya;
          while (istok(*p) && ty - tya < IdntSz - 1) *ty++ = *p++;
          *ty = 0;
          if (*p++ != '>') {
            lineno = i;
        ...
        readnum:
          ...
          if (doty) {
            if (!ty && l > 0) ty = is[r->rhs[l - 1]].type;
            if (!ty || !ty[0]) {
              lineno = i;
              die("$n has no type");
            }
...
void init(int ac, char *av[]) {
  ...
  for (av++; av[0] && av[0][0] == '-'; av++)
    for (opt = &av[0][1]; (c = *opt); opt++) switch (c) {
        case 'v':
          vf = 1;
          break;
        case 'd':
  ...
  if (!(srca = *av)) goto usage;
  fin = fopen(srca, "r");
  if (strlen(pref) + 10 > sizeof buf) die("-b prefix too long");
...
int main(int ac, char *av[]) {
  ...
  if (srconf) fprintf(stderr, "%d shift/reduce conflicts\n", srconf);
  if (rrconf) fprintf(stderr, "%d reduce/reduce conflicts\n", rrconf);
